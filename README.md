...
| Pages show "Network connection lost" / the website can't reach the API                      | A previous dev session left a stale local registry entry. Stop `npm run dev` and start it again: the `predev` step clears it (`scripts/clean-dev-registry.mjs`). |
