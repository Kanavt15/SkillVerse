...
| ---------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **S**poofing identity        | Stolen session cookie, credential stuffing              | HttpOnly `__Host-` cookies (P1), hashed session ids (P1), rate limits on auth (✅ binding ready), Turnstile (P1), 2FA for admins and payees (P1) |
...
| **R**epudiation              | "I never issued that refund"                            | Append-only `audit_logs` with actor, request ID, IP hash (✅ table; entries written from P1)                                                     |
...
| **E**levation of privilege   | Learner calls admin endpoint, forges role               | Roles only from DB, deny-by-default policies (P1), Cloudflare Access in front of `/admin` (P1)                                                   |
